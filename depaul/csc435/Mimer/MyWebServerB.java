...
> javac -cp .;xstream-1.2.1.jar;xpp3_min-1.1.3.4.O.jar MyWebServerB.java

4. Precise examples / instructions to run this program:
> java -cp .;xstream-1.2.1.jar;xpp3_min-1.1.3.4.O.jar MyWebServerB.java
...
class WorkerB extends Thread { // The worker thread which serves connection from JokeClient
	Socket sock;
	WorkerB (Socket s) {sock = s;}	// Get the socket handler.
...
class BCWorkerB extends Thread { // The worker thread which serves connection from BCHandler.
	Socket sock;
	BCWorkerB (Socket s) {sock = s;}	// Get the socket handler.
...
class BCLooperB implements Runnable {
	
	public void run(){ // Running the back-end channel listen loop
	
		int q_len = 6; /* Number of requests for OpSys to queue */
		int port = 2550;  
		// We are listening at a random port from 2550 ~ 2649 as bragging right version.
		Random rand = new Random();
		port = port + rand.nextInt(100);
		Socket sock;
		try{
			ServerSocket servsock = new ServerSocket(port, q_len);
			System.out.println("Backend Channel Looper starting up, listening at random port " + port + " for BCHandler.");
			
			// Generate a .xyz file with the server_ip and server_port information inside.
			String str_file = "mimer-data-random.xyz";
			System.out.println("Writing server_ip and server_port information to " + str_file);
			File xyz_file = new File(str_file);
			if (xyz_file.exists() == true && xyz_file.delete() == false) {
				throw (IOException) new IOException(".xyz file delete failed.");
		    }
			xyz_file = new File(str_file);
			if (xyz_file.createNewFile() == false){
				throw (IOException) new IOException(".xyz file creation failed.");
			}
			else {
				PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(xyz_file)));
				//The first line is the server_ip
				pw.println("server_ip=" + InetAddress.getLocalHost().getHostAddress());
				//The second line is the server_port
				pw.println("server_port=" + port);
				//The five lines
				pw.println("One partridge in a pear tree");
				pw.println("Two turtle doves");
				pw.println("Three French hens");
				pw.println("Four calling birds");
				pw.println("Five golden rings");
				pw.close();
			}
			while (true) {
				sock = servsock.accept();
				new BCWorkerB (sock).start();
			}
		} catch (IOException ioe) {System.out.println(ioe);}
...
public class MyWebServerB {

	public static void main(String[] args) throws IOException {
		BCLooperB BCL = new BCLooperB(); // create a thread for back-end channel connection.
